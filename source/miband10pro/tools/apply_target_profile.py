#!/usr/bin/env python3
"""Apply a validated 336x480 target profile to the legacy layout prototype."""
...
def validate_profile(profile: dict) -> tuple[int, int, int, int, str]:
    required = (
        "schemaVersion", "id", "width", "height", "panelX",
        "panelPadding", "status", "evidence",
    )
    ...
    if width - panel_x - padding - 8 < 80:
        raise ValueError("Panel content width is too small")
...
def main() -> int:
    ...
    text = replace(r"const SCREEN_WIDTH = \d+;", f"const SCREEN_WIDTH = {width};", text, "SCREEN_WIDTH")
    text = replace(r"const SCREEN_HEIGHT = \d+;", f"const SCREEN_HEIGHT = {height};", text, "SCREEN_HEIGHT")
    text = replace(r"const PANEL_X = \d+;", f"const PANEL_X = {panel_x};", text, "PANEL_X")
    text = replace(r"const PANEL_PADDING = \d+;", f"const PANEL_PADDING = {padding};", text, "PANEL_PADDING")
    text = replace(r"const TARGET_PROFILE = '[^']+';", f"const TARGET_PROFILE = '{profile_id}';", text, "TARGET_PROFILE")
    index_path.write_text(text, encoding="utf-8")

    app_path = project / "device/app.js"
    text = app_path.read_text(encoding="utf-8")
    text = replace(r"getPx\(\d+\)", f"getPx({width})", text, "getPx")
    text = replace(r"canvasWidth: \d+", f"canvasWidth: {width}", text, "canvasWidth")
    text = replace(r"canvasHeight: \d+", f"canvasHeight: {height}", text, "canvasHeight")
    text = replace(r"targetProfile: '[^']+'", f"targetProfile: '{profile_id}'", text, "targetProfile")
    ...
    for key in ("alternateTargetProfile", "alternateCanvas", "deviceSource"):
        notes.pop(key, None)
    ...
    if isinstance(platform, dict):
        notes["targetDeviceModel"] = platform.get("deviceModel")
        notes["targetDeviceType"] = platform.get("deviceType")
        notes["targetWatchOS"] = platform.get("watchOS")
        notes["targetResolutionCode"] = platform.get("resolutionCode")
        notes["targetPacket"] = platform.get("packet")
